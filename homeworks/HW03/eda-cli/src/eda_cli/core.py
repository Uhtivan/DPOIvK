...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    flags["has_constant_columns"] = any(summary.df[col].nunique() == 1 for col in summary.df.columns)
    ...
    flags["has_high_cardinality_categoricals"] = any(
        summary.df[col].dtype == 'object' and summary.df[col].nunique() > high_cardinality
        for col in summary.df.columns
    )

    # Флаг для дублирующихся идентификаторов
    flags["has_suspicious_id_duplicates"] = summary.df["user_id"].duplicated().any() if "user_id" in summary.df.columns else False
