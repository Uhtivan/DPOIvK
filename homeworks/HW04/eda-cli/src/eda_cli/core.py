...
def compute_quality_flags(df: pd.DataFrame, missing_df: pd.DataFrame) -> Dict[str, Any]:
    """
    Простейшие эвристики качества данных:
    - Проверка на константные столбцы.
    - Проверка на высокую кардинальность категориальных признаков.
    - Проверка на дублирующиеся идентификаторы.
    """
    flags: Dict[str, Any] = {}

    # Флаг для константных колонок (все значения будут одинаковы)
    flags["has_constant_columns"] = any(df[col].nunique() == 1 for col in df.columns)

    # Флаг для категориальных признаков с высокой кардинальностью
    high_cardinality = 100  # Крайность для кардинальности
    flags["has_high_cardinality"] = any(
        (df[col].dtype == 'object' or pd.api.types.is_categorical_dtype(df[col])) and df[col].nunique() > high_cardinality
        for col in df.columns
    )

    # Флаг для дублирующихся идентификаторов
    flags["has_suspicious_id_duplicates"] = df["user_id"].duplicated().any() if "user_id" in df.columns else False

    # Проверки и вычисление качества данных
    ...
    flags["too_few_rows"] = df.shape[0] < 100
    flags["too_many_columns"] = df.shape[1] > 100

    # Проверка
    score = 1.0
    score -= max_missing  # Чем больше пропусков, тем больше возникает проблем
    if df.shape[0] < 100:
        score -= 0.2
    if df.shape[1] > 100:
        score -= 0.1
    ...
    score = max(0.0, min(1.0, score))  # Ограничение оценки с 0 до 1
