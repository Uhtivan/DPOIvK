...
def report(
    ...
    top_k_categories: int = typer.Option(10, help="Количество (top-k) категорий для категориальных признаков."),
    title: str = typer.Option("EDA Report", help="Заголовок отчёта."),
    min_missing_share: float = typer.Option(0.1, help="Порог возможных пропусков, выше которого заходить нельзя.")
) -> None:
    """
    Сгенерировать полный EDA-отчёт с новыми параметрами:
    """
    ...
    # Качество
    quality_flags = compute_quality_flags(df, missing_df)

    # Сохранение табличных артефактов
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"## Гистограммы числовых колонок\n\n")
        f.write(f"См. файлы `hist_*.png`, максимальное количество столбцов для гистограмм: {max_hist_columns}.\n")
